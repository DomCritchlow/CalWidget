...
bold "2. App Store Connect API key for notarytool"
echo "   This account is a Managed Apple ID, which cannot use an app-specific"
echo "   password for notarization. Create an API key (Developer role) at"
echo "   https://appstoreconnect.apple.com → Users and Access → Integrations,"
echo "   download the .p8, and note its Key ID and the Issuer ID."
echo "   Then store it in keychain under the profile name '$NOTARY_PROFILE':"
echo ""
echo "     xcrun notarytool store-credentials $NOTARY_PROFILE \\"
echo "         --key   AuthKey_XXXXXXXXXX.p8 \\"
